...
//   //If no token is present in the request header return error. This means the user is not logged in.
  if (!token) 
return res.send({ result: false, msg: "token must be present" });
...
//   
  ...
  if (!decodedToken)
    return res.send({ result: false, msg: "token is invalid" });

  let userId = req.params.userId;
  let userDetails = await userModel.findById(userId);
  if (!userDetails)
    return res.send({ result: false, msg: "No such user exists" });

  res.send({ result: true, data: userDetails });

  return res.result(500).json({ result: false, msg: "Internal server error" });
...
const updateUser = async function (req, res) {
  ...
  res.send({ result: updatedUser, data: updatedUser },{bufferTimeOutMs:20000});
};
